...
[VEe,mz,n]=expectationvalue2(mps2,Oset);
[Vcurr,Tcur,n]=expectationvalue2(mps2,OsetCurrent);
[Vcurr2,Tcur2,n]=expectationvalue2(mps2,OsetCurrent2);
Vcurr=Vcurr-Vcurr2;
cur(step)=(Tcur-Tcur2)/(N-1);
mzvalues(step,:)=real(VEe/n);
...
current=mean(SCurrent,1);
Err=std(SCurrent,0,1)/sqrt(Nrea);
...
pcolor(mzvalues'),shading interp
...
II=[20 25 30 35 40];
hold on
errorbar(II,real(Vcurrent),VErr,'o')
hold on
plot(II,VcurrentR,'o')
options = optimset('TolFun',1e-10,'TolFun',1e-10);
[alpha ,Dist] = fminsearch(@(alpha)ffiitt(real(Vcurrent),II,alpha),[-rand rand]...
    ,options);
...
Tams=size(mps{site});
mps{site}=contracttensors(Jump2{site},4,4,mps{site},3,3);
mps{site}=permute(mps{site},[1,2,5,4,3]);
mps{site}=reshape(mps{site},Tams);
b=reshape(mps{site},[prod(size(mps{site})),1]);
K=-b'*b;
mps{site}=mps{site}/sqrt(K);
